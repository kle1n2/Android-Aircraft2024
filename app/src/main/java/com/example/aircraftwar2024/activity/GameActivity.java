    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        handler2 = new Handler(getMainLooper()){
            ...
            public void handleMessage(Message msg){
                if(msg.what==1){
                    baseGameView.setEnemyScore(Integer.parseInt((String) msg.obj));
                } else if (msg.what==2) {
                    baseGameView.mbLoop=false;
        ...
        if(BaseGame.getMulFlag()) {
            new Thread(new NetConn(handler2)).start();
        }
            ...
            try{
                ...
                Thread sendServerMsg =  new Thread(){
                    ...
                    public void run(){
                        while(true)
                        {
                            if(!baseGameView.getGameOverFlag()) {
                                writer.println(Integer.toString(baseGameView.getScore()));
                            }
                            else {
                                writer.println("dead");
                                break;

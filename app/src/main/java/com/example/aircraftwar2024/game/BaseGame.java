...
public abstract class BaseGame extends SurfaceView implements SurfaceHolder.Callback, Runnable{
    ...
    private Boolean bgmFlag=false;
    private Boolean bossBgmFlag=false;
    private static Boolean ifMultiplayer=false;

    public BaseGame(Context context){
        super(context);

        myContext = context;
        mysp = new MySoundPool(myContext);
    ...
    public static void setMulFlag(Boolean bool){
        ifMultiplayer=bool;
    }
    public static Boolean getMulFlag(){
    ...
    public void action() {
        if(!bgmFlag){
            MyMediaPlayer.playBGM(myContext);
            bgmFlag=true;
        }
    ...
    protected List<AbstractEnemyAircraft> produceBoss() {
        ...
        if (this.getScore() >= bossScoreThreshold && !this.existBoss()) {
            ...
            if(!bossBgmFlag){
                ...
                MyMediaPlayer.playBossBGM(myContext);
                bossBgmFlag=true;
            }
        }
        return res;
    ...
    private void postProcessAction() {
        ...
        if (heroAircraft.notValid()) {
            if(bossBgmFlag)  MyMediaPlayer.stopBossBGM();
            MyMediaPlayer.stopBGM();
            mysp.playSound(3);
            gameOverFlag = true;
            Log.i(TAG, "heroAircraft is not Valid");
            if(!ifMultiplayer) {
                Score a = new Score(score, "test");
                ScoreDaoImpl scoreDao = new ScoreDaoImpl(myContext);
                scoreDao.setFileName(GameActivity.getGameType());
                List<Score> leaderboard = scoreDao.readScoresFromFile();
                scoreDao.doAdd(a, leaderboard);
                scoreDao.writeScoresToFile(leaderboard);
                Message message = Message.obtain();
                message.what = 1;
                message.obj = "gameover";
    ...
    private void paintScoreAndLife() {
        ...
        canvas.drawText("Score:" + score ,2,82,scoreText);
        if(ifMultiplayer){
            Paint enemyScoreText = new Paint();
            enemyScoreText.setColor(Color.RED);
            scoreText.setTextSize(50);
            canvas.drawText("Enemy Score:" + score ,30,82,enemyScoreText);
        }
        Paint lifeText = new Paint();
        lifeText.setColor(Color.RED);
        lifeText.setTextSize(50);
        canvas.drawText("Life:" + heroAircraft.getHp() ,2,40,lifeText);
    }
